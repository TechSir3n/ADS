...
namespace ADS {
template <typename Type> class BSTree : public TreeNode<Type> {
  private:
    ...
    using is_base_of = typename std::enable_if_t<std::is_base_of<T, TT>::value,
                                                 bool>; // it's just practice for metaprogramming

  public:
    BSTree() noexcept { root = new Node(3); }

    constexpr BSTree(const std::initializer_list<Type>& t_list) {
        std::for_each(t_list.begin(), t_list.end(), [&](auto& t_element) { insert(t_element); });
    }
  ...
  public:
    ...
    template <typename Func> void postorderPrint(Node* t_node, Func fnc) const noexcept {
        if (t_node != nullptr) {
