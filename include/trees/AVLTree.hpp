...
public:
    ...
    constexpr AVLTree(const std::initializer_list<Type> &t_list) noexcept  {
        std::for_each(t_list.begin(),t_list.end(),[&](auto &element){
            insert(element);
        });
    }
...
public:
   ...
   [[nodiscard]] Node *leftRotation(Node *node) noexcept { // используется когда разность высот правого поддерева больше чем  левого поддерева на 1 
   ...
   [[nodiscard]] Node *rightRotation(Node *node) noexcept {   // используется когда разность высот левого поддерева больше чем на 1 
